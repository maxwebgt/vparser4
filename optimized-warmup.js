...
console.log('🛒 ЭФФЕКТИВНЫЙ ПАРСЕР НАЛИЧИЯ: Копируем логику из scraper.js\n');
...
const parseProductAvailability = async () => {
  ...
  // 📋 [HEADERS] Критически важные заголовки - ТОЧНО КАК В SCRAPER.JS!
  ...
  // 🔧 [USER-AGENT] Реалистичный User-Agent - КАК В SCRAPER.JS
  ...
  // 🎭 [АНТИ-ДЕТЕКЦИЯ] ПОЛНАЯ ПРОФЕССИОНАЛЬНАЯ АНТИ-ДЕТЕКЦИЯ ИЗ SCRAPER.JS
  ...
  // 🏠 [STAGE 1/3] ГЛАВНАЯ СТРАНИЦА - КАК В SCRAPER.JS
  ...
  // 🏙️ [STAGE 2/3] УСТАНОВКА ГОРОДА - КАК В SCRAPER.JS
  ...
  // 🛒 [STAGE 3/3] ПЕРЕХОД НА ТОВАР - КАК В SCRAPER.JS
  ...
  // ПАРСИНГ НАЛИЧИЯ - ТОЧНО КАК В SCRAPER.JS extractProductDataImproved
  console.log('\n🔍 ПАРСИНГ НАЛИЧИЯ...\n');
  ...
  const availabilityData = await page.evaluate(() => {
    ...
    // 1. ИЩЕМ КНОПКУ "В КОРЗИНУ" - ТОЧНО КАК В SCRAPER.JS
    ...
    // Если не нашли специфичные селекторы, ищем по тексту кнопки - КАК В SCRAPER.JS
    if (!addToCartBtn) {
    ...
    if (addToCartBtn) {
      ...
      if (!addToCartBtn.disabled && !addToCartBtn.classList.contains('disabled')) {
        ...
        // 2. ИЩЕМ ТОЧНОЕ КОЛИЧЕСТВО - ТОЧНО КАК В SCRAPER.JS
